...
这是公开 GitHub `v0.1.0` 预发布版本的首次发布日期。
...
当前公开版本为预发布版本 `0.2.0`。该版本新增 standalone 本地启动器、软件架构说明和公开 GUI。软件许可和完整外部数据复现尚未最终确认；条件满足后可发布 `1.0.0`。
...
**[点击 Add an element；以下每一组建立一个独立关联产品]**

#### Linked product 1 — C-MAPSS
...
- **Research product identifier type:** `URL` or, if free text is allowed, `NASA Open Data identifier`
- **Version number:** `FD001–FD004` — the provider does not assign a formal version number; the NASA record was modified on 2024-05-15.
- **License:** `License not specified by the NASA Open Data Portal`

#### Linked product 2 — NGAFID

- **Full name:** `A Large-Scale Annotated Multivariate Time Series Aviation Maintenance Dataset from the NGAFID`
- **Briefly describe the research output:** `Public aviation dataset containing multivariate flight records and maintenance labels. MII-CODE uses pre- and post-maintenance flight information for real-world maintenance-impact identification and validation.`
- **Research product persistent identifier:** `10.5281/zenodo.6624956`
- **Research product identifier type:** `DOI`
- **Version number:** `v1`
- **License:** `Creative Commons Attribution 4.0 International (CC BY 4.0)`

#### Linked product 3 — journal article
...
#### Linked product 4 — conference paper
...
> Open access through the public GitHub repository. The source is publicly viewable, but reuse terms remain undefined until an explicit software licence is approved.

若下拉框只有简化选项，选择 **Open access**。
...
> No explicit software licence was found in the retained source folders. The source is publicly visible on GitHub, but the absence of a software licence means that no general permission to copy, modify or redistribute it has yet been granted. Public reuse requires confirmation of software authorship and rights holders, approval by the contributors and relevant institutions, selection of an explicit software licence, and a compatibility review covering reused libraries, datasets and third-party material. The package does not redistribute raw C-MAPSS or NGAFID data; users must obtain these datasets from their authoritative sources and comply with their original terms. Third-party Python libraries remain subject to their own licences. No personal data or credentials are intentionally included.
...
> Yes — the methods and experimental use have been described in associated scientific publications, and the sanitized standalone `v0.2.0` source package, GUI and architecture documentation are publicly available on GitHub.
...
7. 后续稳定发布采用 `1.0.0` 前所需完成的许可和复现确认。
